...
public final class InitScreenState extends BasicGameState {
	...
	public void render ( GameContainer container, StateBasedGame game, Graphics gr ) throws SlickException {
		...
		gr.clear();
		gr.drawImage( title, ( LudumDare23Game.WIDTH - title.getWidth() ) / 2,
			LudumDare23Game.HEIGHT / 5 - title.getHeight() / 2 );
		
		// Draw a red triangle spinning and some text explaining red bonuses
		Shape sR = VirusType.ATTACKER.getShape()
			.transform( Transform.createRotateTransform( angR ) )
			.transform( Transform.createScaleTransform( 32, 32 ) )
			.transform( Transform.createTranslateTransform( LudumDare23Game.WIDTH/6, LudumDare23Game.HEIGHT - (240) ) );
		...
		gr.drawString( "+25% damage to healthy cells", LudumDare23Game.WIDTH/6 + 64, LudumDare23Game.HEIGHT - 240 - 12 );
		gr.drawString( "+10% bonus damage for triangles to infected cells", LudumDare23Game.WIDTH/6 + 64, LudumDare23Game.HEIGHT - 240 );
		
		// Green Circle now
		Shape sG = VirusType.GENERATOR.getShape()
			.transform( Transform.createRotateTransform( angG ) )
			.transform( Transform.createScaleTransform( 32, 32 ) )
			.transform( Transform.createTranslateTransform( LudumDare23Game.WIDTH/6, LudumDare23Game.HEIGHT - (160) ) );
		...
		gr.drawString( "+10% reproduction bonus for circles", LudumDare23Game.WIDTH/6 + 64, LudumDare23Game.HEIGHT - 160 );
		
		// Blue square at last
		Shape sB = VirusType.DEFENDER.getShape()
